...
// Initialize obvious default values here (e.g. 0 for pointers)
    : OpenGLWindow(parent), modelMesh(0),
      ...
      isGPGPU(true), hasComputeShaders(true), blinnPhong(true), transparent(true), eta(1.5), etaImg(1.5),lightIntensity(1.0f), shininess(50.0f), lightDistance(5.0f), groundDistance(0.78),
...
void glShaderWindow::resetAccumulationBuffers()
{
    int bufferWidth = width();
    int bufferHeight = height();

    // Zero-filled arrays
    std::vector<float> zeroAccum(4 * bufferWidth * bufferHeight, 0.0f);
    std::vector<uint32_t> zeroCount(bufferWidth * bufferHeight, 0);

    // accumData
    glBindBuffer(GL_SHADER_STORAGE_BUFFER, ssbo[5]);
    glBufferSubData(GL_SHADER_STORAGE_BUFFER, 0,
        zeroAccum.size() * sizeof(float),
        zeroAccum.data());

    // sampleCount
    glBindBuffer(GL_SHADER_STORAGE_BUFFER, ssbo[6]);
    glBufferSubData(GL_SHADER_STORAGE_BUFFER, 0,
        zeroCount.size() * sizeof(uint32_t),
        zeroCount.data());
...
void glShaderWindow::updateEtaImg(int etaImgSliderValue)
{
    etaImg = etaImgSliderValue/100.0;
    if (compute_program) {
        resetAccumulationBuffers(); 
    }
    //renderNow();
...
QWidget *glShaderWindow::makeAuxWindow()
{
    ...
    QGroupBox *groupBox2 = new QGroupBox("Noise:");
    QRadioButton *transparent1 = new QRadioButton("&Halton");
    QRadioButton *transparent2 = new QRadioButton("&Random");
    ...
    // EtaImg slider
    QSlider* etaImgSlider = new QSlider(Qt::Horizontal);
    etaImgSlider->setTickPosition(QSlider::TicksBelow);
    etaImgSlider->setTickInterval(100);
    etaImgSlider->setMinimum(0);
    etaImgSlider->setMaximum(500);
    etaImgSlider->setSliderPosition(etaImg*100);
    connect(etaImgSlider,SIGNAL(valueChanged(int)),this,SLOT(updateEtaImg(int)));
    QLabel* etaImgLabel = new QLabel("Eta img (index of refraction) * 100 =");
    QLabel* etaImgLabelValue = new QLabel();
    etaImgLabelValue->setNum(etaImg * 100);
    connect(etaImgSlider,SIGNAL(valueChanged(int)),etaImgLabelValue,SLOT(setNum(int)));
    QHBoxLayout *hboxEtaImg= new QHBoxLayout;
    hboxEtaImg->addWidget(etaImgLabel);
    hboxEtaImg->addWidget(etaImgLabelValue);
    outer->addLayout(hboxEtaImg);
    outer->addWidget(etaImgSlider);
...
void glShaderWindow::createSSBO() 
{
 /* Build BVH */
    ...
    // Generate 7 buffers for various SSBOs
    glGenBuffers(7, ssbo);

    // Vertices Buffer
    glBindBuffer(GL_SHADER_STORAGE_BUFFER, ssbo[0]);
    glBufferData(GL_SHADER_STORAGE_BUFFER, modelMesh->vertices.size() * sizeof(trimesh::point), 
                 &(modelMesh->vertices.front()), GL_STATIC_READ);
    glBindBufferBase(GL_SHADER_STORAGE_BUFFER, 1, ssbo[0]);

    // Normals Buffer
    glBindBuffer(GL_SHADER_STORAGE_BUFFER, ssbo[1]);
    glBufferData(GL_SHADER_STORAGE_BUFFER, modelMesh->normals.size() * sizeof(trimesh::vec), 
                 &(modelMesh->normals.front()), GL_STATIC_READ);
    glBindBufferBase(GL_SHADER_STORAGE_BUFFER, 2, ssbo[1]);

    // Colors Buffer
    glBindBuffer(GL_SHADER_STORAGE_BUFFER, ssbo[2]);
    glBufferData(GL_SHADER_STORAGE_BUFFER, modelMesh->colors.size() * sizeof(trimesh::Color), 
                 &(modelMesh->colors.front()), GL_STATIC_READ);
    ...
    glBufferData(GL_SHADER_STORAGE_BUFFER, triangles.size() * sizeof(Triangle), 
                 triangles.data(), GL_STATIC_READ);
    glBindBufferBase(GL_SHADER_STORAGE_BUFFER, 4, ssbo[3]);

    // BVH Nodes Buffer
    glBindBuffer(GL_SHADER_STORAGE_BUFFER, ssbo[4]);
    glBufferData(GL_SHADER_STORAGE_BUFFER, nodes.size() * sizeof(BVH::Node), 
                 nodes.data(), GL_STATIC_READ);
    glBindBufferBase(GL_SHADER_STORAGE_BUFFER, 5, ssbo[4]);

    // Accumulation Buffer
    int bufferWidth = this->width();
    int bufferHeight = this->height();

    // 1) Create a float vector for RGBA accumulation (4 floats per pixel)
    std::vector<float> initialAccumData(4 * bufferWidth * bufferHeight, 0.0f);

    // 2) Transfer that data to the SSBO
    glBindBuffer(GL_SHADER_STORAGE_BUFFER, ssbo[5]);
    glBufferData(GL_SHADER_STORAGE_BUFFER,
                 initialAccumData.size() * sizeof(float),
                 initialAccumData.data(),
                 GL_DYNAMIC_COPY);
    glBindBufferBase(GL_SHADER_STORAGE_BUFFER, 6, ssbo[5]);

    // Sample Count Buffer
    glBindBuffer(GL_SHADER_STORAGE_BUFFER, ssbo[6]);
    std::vector<uint32_t> sampleCount(bufferWidth * bufferHeight, 0);
    glBufferData(GL_SHADER_STORAGE_BUFFER,
    ...
    glBindBuffer(GL_SHADER_STORAGE_BUFFER, ssbo[7]); // Use the next available slot in ssbo array

    // Initialize with zeros (assuming float data for variance calculations)
    std::vector<float> varianceBuffer(bufferWidth * bufferHeight, 0.0f);

    // Allocate and upload data to the buffer
    glBufferData(GL_SHADER_STORAGE_BUFFER,
                varianceBuffer.size() * sizeof(float),
                varianceBuffer.data(),
                GL_DYNAMIC_COPY);

    // Bind the buffer to binding point 8
    glBindBufferBase(GL_SHADER_STORAGE_BUFFER, 8, ssbo[7]);
    ...
    // Bind SSBOs to the compute shader program
...
void glShaderWindow::loadTexturesForShaders() {
    ...
    if (hasComputeShaders) {
        // bind the texture generated to texture unit 2 (0 is for the texture, 1 for the env map)
...
void glShaderWindow::render()
{
    ...
    if (hasComputeShaders) {
        ...
        compute_program->setUniformValue("etaImg", etaImg);
        compute_program->setUniformValue("framebuffer", 2);
        compute_program->setUniformValue("colorTexture", 0);

        glBindImageTexture(0, computeResult->textureId(), 0, false, 0, GL_WRITE_ONLY, GL_RGBA32F);
        compute_program->setUniformValue("framebuffer", 0);

        int worksize_x = (width() + compute_groupsize_x - 1) / compute_groupsize_x * compute_groupsize_x;
        int worksize_y = (height() + compute_groupsize_y - 1) / compute_groupsize_y * compute_groupsize_y;
    ...
    m_program->setUniformValue("etaImg", etaImg);
    ...
    if (m_program->uniformLocation("shadowMap") != -1) {
        ...
        // shadowMapping: send the right transform here
    }
